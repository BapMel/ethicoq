...
(** Tactique de Yann Régis-Gianas pour effectuer des démonstrations
par synthèse. *)
(* Tactic Notation "have" ident(H) "by" constr(L) :=                                 
   generalize L; intro H. *)

Tactic Notation "have" simple_intropattern(pattern) "by" constr(L) :=
   generalize L; intros pattern.

(* Tactic Notation "synth" constr(L) "as" simple_intropattern(pattern) :=                                  *)
(*    generalize L; intros pattern.  *)

(* Tactic Notation "have" ident(H) "by" constr(L) "as" simple_intropattern(pattern) :=                                  *)
(*    generalize L; intros H; destruct H as pattern.  *)
...
Module Pars1.
  ...
  Variable ejusdem_naturae: aliquid -> aliquid -> Prop.
  ...
  Hypothesis Pars1_definitio2: 
    forall a1: aliquid, 
      ...
      (exists a2: aliquid,
          a1 <> a2
          /\ ejusdem_naturae a1 a2
          /\ terminari_potest a1 a2).
...
have [Pars1_def5_a [Pars1_def5_b Pars1_def5_c]] by (Pars1_definitio5 m s).
(* synth (Pars1_definitio5 m s) as [Pars1_def5_a [Pars1_def5_b Pars1_def5_c]]. *)

(* generalize (Pars1_definitio5 m s). intros [H1 [H2 H3]].
intro H.
have HR by (Pars1_definitio5 m s).
have HR by (Pars1_definitio5 m s) as [H1 [H2 H3]]. *)

have [Mm [Ss existit_in_m_s]] by (Pars1_def5_a ms).
have prior_s_m by (existit_in_prior s m existit_in_m_s).
assumption.
...
(* XXXX affaiblir cet axiome ? La notion de concevoir est à préciser. -- Ch. J. *)
...
Qed.
  ...
  différentes (par exemple substance et mode) et alors c'est trivial
...
appeler à la proposition 1, la définition 3 et l'axiome 6. *)
  ...
  Proof. 
    ...
    (** Spinoza invoque à juste titre la proposition 3 : deux choses qui
...
(* Hypothesis ejusdem_naturae_substantia: *)
(*   forall s1 s2: aliquid, *)
(*     substantia s1 -> ejusdem_naturae s1 s2 -> substantia s2. *)


(* Hypothesis terminari_potest_ergo_communes_attributa: *)
(*   forall a1 a2: aliquid, *)
(*     terminari_potest a1 a2 -> *)
(*     substantia a1 /\ *)
(*     substantia a2 /\ *)
(*     a1 <> a2 /\ *)
(*     (exists a: aliquid, *)
(*        attributum_substantiæ a a1 /\ *)
(*        attributum_substantiæ a a2). *)


(** Spinoza a l'air de présupposer l'axiome suivant dans la
démonstration de la proposition 8 : « deberet terminari ab alia ejusdem
naturæ [...] adeoque darentur duæ substantiæ ejusdem attributi ». De
même, l'énoncé de la proposition 5 contient : « substantiæ ejusdem
naturæ SIVE attributi ». *)

Hypothesis ejusdem_naturae_ergo_communes_attributa:
  forall a1 a2: aliquid,
    terminari_potest a1 a2 ->
    substantia a1 /\
    substantia a2 /\
...
(* Hypothesis terminari_potest_ergo_ejusdem_naturae: *)
(*   forall a1 a2: aliquid, *)
(*     terminari_potest a1 a2 -> *)
(*     ejusdem_naturae a1 a2. *)
...
Proof.
  ...
  (** Supposons une substance finie en son genre. *)

  intros s S finita.

  
  (** Nous allons montrer par l'absurde qu'elle est bornée par une autre
  de même nature, alors que c'est impossible. *)
  
  absurd (exists s2: aliquid,
            s <> s2 /\
            ejusdem_naturae s s2 /\
            terminari_potest s s2).

(* Ch. J. : infini en son genre n'est pas absolument infini *)

  (** Commençons par démontrer qu'elle ne peut pas être bornée par une
  autre de même nature. *)
  
  intro terminari_potest.
  destruct terminari_potest
    as [s2 [diff [ejusdem_naturae_s_s2 substantia_terminari]]].

  (** Si elle est de même nature alors les deux substances ont des
  attributs communs. *)
  
    destruct ejusdem_naturae_ergo_communes_attributa
  with (a1 := s) (a2 := s2) as [Ss [Ss2 [diff2 attributa]]].
  exact substantia_terminari.

  (** Or, par la proposition 5, deux substances ne peuvent avoir des
  attributs communs. *)
  
  apply Pars1_propositio5 with (s1 := s) (s2 := s2).
  exact S. exact Ss2. exact diff. exact attributa.


 (** Montrons maintenant qu'une substance finie en son genre peut être
bornée par une autre de même nature : c'est exactement la définition 2.
  *)
 
 apply Pars1_definitio2.
 exact finita.

Qed.

(** La proposition est démontrée sans que l'on ait eu à utiliser la
proposition 7 : nul besoin de poser l'existence de l'une ou l'autre des
deux substances. *)
